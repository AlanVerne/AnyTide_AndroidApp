...
public class SunTimesProvider {
    ...
    public static SunTimes get(LatLng position, TimeZone timeZone) {
        long time = Common.getDay(0, timeZone);
        return get(position, time, timeZone);
    }
    public static SunTimes get(LatLng position, long date, TimeZone timeZone) {
        double  lw = -position.longitude * deg2rad,
                phi = position.latitude * deg2rad,
                J = unixToJulian(date),
